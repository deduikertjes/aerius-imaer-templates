    ...
    {% block featureMembers %}
        ...
        <imaer:OffRoadMobileSourceEmissionSource sectorId="{{ f['sectorId'] }}" gml:id="{{ f['gmlId'] }}">
            <imaer:identifier>
                <imaer:NEN3610ID>
...
{{ f['geometry'] }}
            {% for is in f['offRoadMobileSources'] %}
            <imaer:offRoadMobileSource>
                <imaer:StandardOffRoadMobileSource offRoadMobileSourceType="{{ is['offRoadMobileSourceType']}}">
                    <imaer:description>{{ is['description']}}</imaer:description>
                    <imaer:literFuelPerYear> {{is['literFuelPerYear']}}</imaer:literFuelPerYear>
